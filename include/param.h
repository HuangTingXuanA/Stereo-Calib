...
struct EdgeParams {
    ...
    int gradThresh = 35;       // 梯度阈值
    int anchorThresh = 15;      // 锚点阈值
    int scanInterval = 1;      // 扫描间隔
    int minPathLen = 5;       // 最小路径长度
 ...
 */
struct ArcParams {
    double epsilon = 0.6;      // RDP 多边形逼近阈值
    double sharpAngle = 100.0;  // 尖角阈值（度）
    int minArcLength = 5;     // 最小圆弧长度
    int threads = 8;           // 并行线程数
};
...
struct FitParams {
    // ========== 配对约束参数 ==========
    double thLengthRatio = 5.0;    // 弧长比限制
    double thDistance = 1.2;       // 距离限制系数
    
    // ========== 评分参数 ==========
    double minEdgeScore = 0.33;    // 最小边缘评分
    double minEllipseScore1 = 0.35; // 椭圆评分阈值1（内点率）
    double minEllipseScore2 = 0.35;// 椭圆评分阈值2（覆盖率）
    double inlierDist = 1.5;       // 内点距离阈值
    
    // ========== 验证参数 ==========
    double remainScore = 0.8;     // 梯度验证保留分数阈值
    ...
    int brightCenterThresh = 100;  // 若 polarity=1: 检查 Center > brightCenterThresh
