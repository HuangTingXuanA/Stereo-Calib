...
struct EdgeParams {
    ...
    int gradThresh = 5;       // 梯度阈值
    int anchorThresh = 5;      // 锚点阈值
    int scanInterval = 1;      // 扫描间隔
    int minPathLen = 1;       // 最小路径长度
    double sigma = 0.8;        // 高斯平滑参数（0: 禁用, >0: 启用并指定标准差）
};
 ...
 */
struct ArcParams {
    double epsilon = 0.8;      // RDP 多边形逼近阈值
    double sharpAngle = 120.0;  // 尖角阈值（度）
    int minArcLength = 1;     // 最小圆弧长度
    int threads = 8;           // 并行线程数
};
...
struct FitParams {
    // ========== 配对约束参数 ==========
    double thLengthRatio = 4.0;    // 弧长比限制
    double thDistance = 1.5;       // 距离限制系数
    
    // ========== 评分参数 ==========
    double minEdgeScore = 0.35;    // 最小边缘评分
    double minEllipseScore1 = 0.3; // 椭圆评分阈值1（内点率）
    double minEllipseScore2 = 0.35;// 椭圆评分阈值2（覆盖率）
    double inlierDist = 1.0;       // 内点距离阈值
    
    // ========== 验证参数 ==========
    double remainScore = 0.5;     // 梯度验证保留分数阈值
    int sampleNum = 72;           // 椭圆采样点数
    int gradRadius = 1;            // 梯度平均半径
    double gradAngleThresh = 15.0; // 梯度角度阈值（度）
    ...
    double minAspectRatio = 0.3;   // 最小长短轴比（偏心率约束）
    int centerIntensityThresh = 0; // 圆心灰度阈值（0:自适应）
...
struct RefineParams {
    ...
    double roiRatio = 1.5;         // ROI 截取比例（相对于长轴）
    int refineIter = 10;            // 迭代次数
    double tukeyAlpha = 4.0;       // Tukey Loss 常数
    int minGradient = 20;          // 最小梯度幅值阈值
